...
例えば`uint8`は８ビットしか持たないため、ここに格納できる最大数値は二進数の`11111111`だ(または十進数だと 2^8 - 1 = 255 となる)。
...
このような場合、オーバーフローとなってしまう。つまり`number`を増やしているのに`0`になるという通常の感覚に反することが起こる(もし二進数の`11111111`に１を足すと、`00000000`に戻ってしまう。時計が`23:59`から`00:00`になってしまうような感じだ)。
...
例えばSafeMathライブラリで、`using SafeMath for uint256`のシンタックスを使うとしよう。SafeMathライブラリは`add`、`sub`、`mul`さらに`div`の４つの関数を持つ。すると以下のように、`uint256`からこれらの関数にアクセス可能だ:
